...
//비동기 데이터의 상태에 따라서 자동으로 해당 데이터를 변경할수 있는 리듀서라는 함수 등록

import axios from 'axios';
import { createAsyncThunk, createSlice } from '@reduxjs/toolkit';

//비동시 서버통신으로 데이터를 전달받아서 내부적으로 action타입을 자동 생성해 액션객체 생성까지 완료하는 함수
...
/**
  리덕스에서 전역상태 관리에 쓰이는 용어 정리

  store : 전역 state 저장 공간 (금고)
  reducer : store의 전역 state값을 변경해주는 함수 (변형자) (금고관리인, 점장)
  dispatch : reducer에 데이터 변경 요청을 해주는 함수 (전달자) (창구직원)
  action : dispatch로 리듀서에 데이터 변경요청을 위해 필요한 특별한 형태의 객체 (창구직원에게 전달하기 위한 자료용지)
