...
/**
	1. 해당 컴포넌트에 대해 설명, 이슈사항
	A:
	-kakao map API가 react버전의 사용구문이 없었기 때문에 일반 cdn방식으로 api를 react구조에 맞게 변환하는 작업이 힘들었다

	이슈사항1 : cdn으로 받아온 kakao 생성자 함수를 컴포넌트 안쪽에서 불러와지지 않는 문제
	ㄴ 직접 window객체로부터 직접 비구조화할당으로 뽑아와서 활용
	- kakao생성자를 통해서 만들어진 지도 인스턴스값을 state에 담아서 각각의 이벤트에 연결함

	(아래 내용 이해 안되면 버튼 빼야함)
	- 작업을 하다보니 암래도 react로 작업하는 프로젝트는 지점이 많은 대형 프로젝트일것 같아서 지점버튼 클릭시 다른 지도를 출력하도록 구현해봤는데 너무 코드가 지저분해져서 지도 정보값을 배열 형태로 묶어서 추후 배열 데이터가 변경이 되면 데이터 기반으로 자동으로 새로운 지도 인스턴스 생성과 이벤트 연결까지 한번에 처리되도록 자동화시키는데 중점을 뒀다

	이슈사항2 : 브라우저 리사이즈시 마커가 가운데 가지 않아서 브라우저 리사이즈 이벤트가 발생할 때마다 마커가 가운데에 위치하는 함수를 재호출함. 
	window객체에 이벤트를 연결하다보니 리사이즈 이벤트가 발생할 필요가 없는 다른 컴포넌트에서도 핸들러 함수가 호출되는 문제점이 있어서 컴포넌트 언마운트시 윈도우 객체에 이벤트 핸들러를 제거함
 */
